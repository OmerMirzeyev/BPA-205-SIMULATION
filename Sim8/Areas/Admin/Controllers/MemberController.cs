...
using Sim8.Areas.Admin.ViewModels.Member;
using Sim8.DAL;
using Sim8.Models;
using Sim8.Utilities;
using System.Threading.Tasks;

namespace Sim8.Areas.Admin.Controllers
{
    [Area("Admin")]
    ...
    public class MemberController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateMemberVM memberVM)
        {
            ...
            {
                ModelState.AddModelError("ImageFile", "ImageFile is valid");
            ...
            else
            {
                ...
                {
                    ModelState.AddModelError("ImageFile", "ImageFile not correct");
                    return View(memberVM);
                }
                if(memberVM.ImageFile.Length > 2 * 1024 * 1024)
                {
                    ModelState.AddModelError("ImageFile", "ImageFile size 2mb");
                    return View(memberVM);
                }
            }
        ...
        public async Task<IActionResult> Update(UpdateMemberVM memberVM)
        {
            ...
            {
                member.ImageUrl = memberVM.ImageFile.SaveImage(_env, "uploads/member");
            }
            await _db.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
